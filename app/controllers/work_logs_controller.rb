class WorkLogsController < ApplicationController
  ...
  def new
    ...
    @entries = current_user.assignments.where('tasks.start_date <= ? AND tasks.end_date >= ?', @date.end_of_day, @date.beginning_of_day)
  ...
  def edit
    @date = @work_log.date
    @entries = Task.where('start_date <= ? AND end_date >= ?', @work_log.date.end_of_day, @work_log.date.beginning_of_day)
    @hours = @work_log.minutes.to_i / 60
    @mins = @work_log.minutes.to_i % 60
  end
  ...
  def delete_request
    flash[:notice] = if @work_log.user_id == current_user.id && @work_log.update_attribute(:delete_request, true)
                       'Delete Request has been made.'
                     else
                       'Delete Request could not be made.'
                     end
    respond_to do |format|
      format.html { redirect_to reports_worklogs_path }
      format.json { head :no_content }
  ...
  def ignore_request
    flash[:notice] = if current_user.manager? && @work_log.update_attribute(:delete_request, false)
                       'Delete Request has been ignored.'
                     else
                       'Delete Request could not be ignored.'
                     end
    respond_to do |format|
      format.html { redirect_to reports_worklogs_path }
      format.json { head :no_content }
