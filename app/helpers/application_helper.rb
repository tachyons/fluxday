module ApplicationHelper
  ...
  def build_validated_field(form_name, field, name, _message, pattern, type, value)
    html = ''
  ...
  def build_field(form_name, field, name, type, value)
    ...
    html << '</div>'
    html << "<div class='small-9 columns'>"
    html << if type == 'password'
              password_field_tag("#{form_name}[#{field}]", value, class: 'text_field', placeholder: name)
            elsif type == 'file'
              file_field_tag("#{form_name}[#{field}]", class: 'text_field')
            elsif type == 'radio'
              radio_button_tag("#{form_name}[#{field}]", value, class: 'text_field', placeholder: name)
            elsif type == 'checkbox'
              check_box_tag("#{form_name}[#{field}]", value, class: 'text_field', placeholder: name)
            elsif type == 'text_area'
              text_area_tag("#{form_name}[#{field}]", value, class: 'text_field', placeholder: name, rows: 1)
            else
              text_field_tag("#{form_name}[#{field}]", value, class: 'text_field', placeholder: name)
            end
    html << '</div>'
    html << '</div>'
    html.html_safe
  ...
  def build_validated_select(form_name, field, name, _message, _pattern, multiple, value, options)
    html = ''
    html << "<div class='form-row'>"
    html << "<div class='small-3 columns'>"
    html << label_tag("#{form_name}_#{field}", name, class: 'left inline')
