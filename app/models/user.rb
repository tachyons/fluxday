class User < ApplicationRecord
  ...
  def manager?
    role.downcase.in?(%w[manager admin])
  end

  def employee?
    role.casecmp('employee').zero?
  end
  ...
  def ensure_manager_exists
    if User.manager_user.count == 1 && changes.present? && changes[:role].present?
      if changes[:role][0] == 'admin' && changes[:role][1] == 'Manager'
        self.role = 'admin'
        true
      elsif (changes[:role][0] == 'admin' && changes[:role][1] == 'Employee') || (changes[:role][0] == 'Manager' && changes[:role][1] == 'Employee')
